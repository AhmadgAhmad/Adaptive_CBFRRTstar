...
class CBF_RRTstrr(object):
    ...
    def Near(self,  b_raduis, Vertex = None, xyCoor = None):
        ...
        xyCoorVertices = xyCoorVertices[:,:,0]
    ...
    def SafeSteering(self, v_nearest, desired_theta, m=10):
        ...
        self.simObject.add_agent(Agent(xy_v_nearest, radius=.5,theta=desired_theta ,instructs=u_ref, dynamics=Dyn.UNICYCLE))
        ...
        qFinal = np.array([[qFinal[0]],[qFinal[1]],[qFinal[2]]])
    ...
    def feasCheck(self, obst, xy_p):
        """

        :param obst: Obstacle object type (ellipsoid or sphere)
        :param xy_p: A 2D point in the Euclidean space
        :return: True if the point lies outside the obs, false otherwise
        """
        if isinstance(obst,Ellipsoid):
            M_ellip = obst.M_ellip
            h = ((xy_p - obst.state[0:2]).transpose().dot(M_ellip).dot((xy_p- obst.state[0:2]))[0][
                     0] - 1) * obst.sign
            if h<0:
                flagFeas = False
            else:
        ...
        if isinstance(obst,Sphere):
            h = (xy_p[0] - obst.state[0]) ** 2 + (xy_p[1] - obst.state[1]) ** 2 - (obst.radius + .7) ** 2
            if h<0:
                flagFeas = False
            else:
                flagFeas = True
    ...
    def Plan_CBF_RRT_strr(self,initTree_flag = False):
        ...
        while i <= self.NSampling or not (reachedFlag): #TODO change
            ...
            xy_p = np.array([[3],[3.5]])
            dFlag = self.feasCheck(self.obsWorldList[0], xy_p)
