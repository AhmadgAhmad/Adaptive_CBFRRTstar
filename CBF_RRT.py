...
class CBF_RRTstrr(object):
    ...
    def ChooseParent(self, v_nearest, v_new):
        """
        ...
        norm_v_pr2v_near = self.Cost_v2v(v_nearest,v_new) #This is the Euclidean distance we'll use arcLength afterwards; this is following the assumption
        ...
        xy_v_near = v_nearest.State[0:2]
        desired_theta = math.atan2(xy_v_pr[1] - xy_v_near[1] , xy_v_pr[0] - xy_v_near[0])
        if mSteering>1:
                qFinal, tFinal, uTrajectory, qTrajectory, tTrajectory = self.SafeSteering(v_nearest,desired_theta=desired_theta,m=mSteering) #TODO (cbfRRT*)
        ...
        if steering_okFlag:
                ...
                norm_v_pr2v_near = self.Cost_v2v(v_nearest,v_new)  # This is the Euclidean distance we'll use arcLength afterwards; this is following the assumption
                c_pr_heuristic = v_nearest.CostToCome + norm_v_pr2v_near
    ...
    def Plan_CBF_RRT_strr(self,initTree_flag = False):
        ...
        while i <= self.NSampling or not (reachedFlag): #TODO change
            ...
            #RRT
            v_min, v_new = self.ChooseParent(v_nearest=v_nearest, v_new=v_new)
            self.addChild(v_min,v_new) # (v_parent,v_child)


            # ===== Inspecting v_goal =============
            if not self.params.dataGen_enabled:
                if (self.initTree_done)  or (not self.params.kde_enabled and not self.params.CE_enabled):
                ...
                if False:#actual_i % 30 == 0:#actual_i == 10 or actual_i == 20 or actual_i==100 or actual_i==150 or actual_i==200 or actual_i==1000:
            ...
            if flag2000:#flag10000 or flag1000 or flag2500 or flag5000 or flag100 or flag500:
                #Save the costs:

                #The first element of the saved list contains the iteration that the goal has been reached at.
                saveData([self.iGoalReached,self.goal_costToCome_list], self.prefix+'CBF_RRT_Cost', suffix=self.suffix,CBF_RRT_strr_obj=self,
                         adapDist_iter=None, enFlag=True)

                saveData(self.iterTime_list, self.prefix + 'CBF_RRT_iterTime',
                         suffix=self.suffix, CBF_RRT_strr_obj=self,
                         adapDist_iter=None, enFlag=True)
